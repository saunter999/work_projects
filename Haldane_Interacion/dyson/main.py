...
def Measure(para, Observable,Factory, G0, W0, T0, G, W, SigmaDeltaT, Sigma, Polar, Determ, ChiTensor):
    log.info("Measuring...")
    ...
    Chi.FFT("R","T")
    #for i in range(6*6):
    #   Ei,Ui=np.linalg.eig(Chi.Data[0,:,0,:,i,0])
    #   print  i,Ei
    ...
    #print "Sigma=\n", Sigma.Data[UP,0,DOWN,0,0,:]
    #print "Chi=\n", Chi.Data[0,0,0,0,1,:]
    #print "------------------------------"
    #print "Chi=\n", Chi.Data[0,0,0,0,0,0]
...
def Dyson(IsDysonOnly, IsNewCalculation, EnforceSumRule, para, Map, Lat):
    ...
    while True:
	    ...
	    log.info("everything is going well!")
            Gold, Wold = G, W
            Measure(para, Observable, Factory, G0, W0, T0, G, W, SigmaDeltaT, Sigma, Polar, Determ, ChiTensor)
