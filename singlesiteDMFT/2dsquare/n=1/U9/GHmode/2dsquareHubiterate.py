...
def DMFT_SCC(mu):
    """This subroutine creates Delta.inp for ctqmc on 2d square lattice.
...
def Delta_start():
	...
	"""
    weave.inline(code,['masbf','Ekls','glatt','delta','mu','Eimp','totk'],type_converters=weave.converters.blitz,force=1,compiler='gcc')
...
def Delta_update():
    ...
    glatt=zeros(Nmats,dtype=complex)
    code="""
	# line 119 "2dsquareHubiterate.py"
	using namespace std;
	double ef=Eimp;
	for (int i=0;i<masbf.size();i++){
	    for(int j=0;j<Ekls.size();j++){
...
def int_mu(mu,sigma):
    Ne=nume_mats(mu,sigma)
    return Ne-Nocc

def nume_mats(chem,sigma):
	Ne=zeros(1)
	gnontk=0.0j
        gintk=0.0j
	code="""
	#line 152 "2dsquareHubiterate.py"
	using namespace std;
	#include <math.h> 
	for (int i=0;i<Ekls.size();i++){
	      Ne(0)=Ne(0)+1./( exp(beta*(Ekls(i)-chem))+1.0);
	      for(int j=0;j<(masbf.size()-1);j++){
		 gnontk=1.0/( masbf(j)+chem-Ekls(i) );
 	         gintk=1.0/(masbf(j)+chem-Ekls(i)-sigma(j));
		 Ne(0)=Ne(0)+2.0/beta* real(gintk-gnontk); 
	      }
        }
	Ne(0)=Ne(0)*2.0/totk;
	"""
        weave.inline(code,['Ne','gnontk','gintk','masbf','Ekls','sigma','chem','totk','beta'],type_converters=weave.converters.blitz,compiler='gcc')
	return Ne[0]
...
if __name__=="__main__":
    ...
    t,Uc=1.0,9.0
    Nk=300
    dlt,mixr=1.0e-2,0.9 ##mu mixing ratio
    totk=(Nk-0.0)**2
    mu=Uc/2. ##for half filling
    ...
    ## 1.setting up for noninteracting band dispersion,mu and dos##
    Ekls=[]
    kmesh=linspace(-pi,pi,Nk,endpoint=False) ##important to excluding the endpoint
    for kx in kmesh:
	for ky in kmesh:
	    Ekls.append( -2.0*t*(cos(kx)+cos(ky) ) )
    Ekls=array(Ekls) 
...
#	    print it,"th iteration"
            Sigit=Read_sigout()
#	    print it,'Determine mu using Gint on imaginary axis'
#	    mu_old=mu
#	    mu=optimize.brentq(int_mu,mu_old-Uc/2.0,mu_old+Uc,args=(Sigit,))
#	    mu=mu*mixr+mu_old*(1.0-mixr)
#	    print 'mu is equal to',mu
#	    params["mu"][0]=mu
#	    CreateInputFile(params)
	DMFT_SCC(mu)
