...
def Delta_start():
    ...
    code="""
	# line 103 "2dsquareHubiterate.py"
	using namespace std;
	double ef=Eimp;
...
if __name__=="__main__":
    ...
    totk=(Nk-0.0)**2
    mu=Uc/2.-0.5 ##initial guess for mu,better choice???
    ...
    fmu=open("mu.dat",'w')
    print>>fmu,"#iteration,mu,Ne-Nocc"
	...
	if(it>0):
	    ...
	    diff=abs(Ne-Nocc) 
	    print>>fmu,it,mu,Ne-Nocc
	    #if diff>0.01:
	    #   mu=optimize.brentq(mu_update,mu_old-Uc*diff,mu_old+Uc*diff)
	    mu=optimize.brentq(mu_update,mu_old-Uc,mu_old+Uc)
