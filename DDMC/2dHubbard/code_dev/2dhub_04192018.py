...
def Grandpotential(n,ekls):
	...
	if n==2:
	   ...
	   print>>f,"Nit||sign||norm||sign*norm||omega"
	   for Nitt in Nls:
	       print "Nitt=",Nitt
	       sign,norm=MCMC(Nitt)
	       print sign,norm,sign*norm,-U**2/2.0*sign*norm
	       print>>f,Nitt,sign,norm,sign*norm,-U**2/2.0*sign*norm
	   omega=-U**2/2.0*sign*norm
...
def MCMC(Nitt):
	...
	norm=Samplenorm(Nitt,warm,measure,Dorder_init,c_norm)
	return sign,norm
	

def Samplenorm(Nitt,warm,measure,Dorder_init,c_norm):
	N0=0.0
        N1=0.0
        Dorder=Dorder_init
        Naver=0.0
        ...
        for itt in range(Nitt):
		   ...
		   ix=random.randint(0,N);iy=random.randint(0,N)
                   P=min([beta*N**2*abs(kernelMr_set(itau,ix*(N-1)+iy))/c_norm,1])
                   ...
                   P=min([c_norm/(beta*N**2*abs(kernelMr_set(itau,ix*(N-1)+iy))),1])
                   if P>rand():#Metropolis
                    #accept:
                        Dorder=0
                        acc2+=1
...
#	     print itt,tau,kernelMr_set(tau,[1,0]),G0rtau_gen(tau,[1,0])
	     for ix in range(N):
	     	for iy in range(N):
		   Mtau+=kernelMr_set(itau,ix*(N-1)+iy)
             sum+=Mtau.real
