...
extern "C" {
  ...
  void FNAME(zheev)(const char* jobz,  const char* uplo,   const int* n,  dcomplex* A, const int* lda, double* w, dcomplex* work, const int* lwork, double* rwork, int* info);
...
// Eigenvalues of complex hermitian matrix
inline int xsyev(const int N, dcomplex* A, const int lda, double* w, dcomplex* work, const int lwork, double* rwork, string job="V")
{
  int info = 0;
  FNAME(zheev)(job.c_str(), "U",  &N,  A, &lda, w, work, &lwork, rwork, &info);
  if (info)
    std::cerr << "Can't compute eigenvalues! " << info << std::endl;
  return info;
}
