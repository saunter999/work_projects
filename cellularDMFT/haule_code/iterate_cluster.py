...
def AssembleHk(ikx,iky,c,s):
    ...
    """
    Hk = zeros((4,4), dtype=complex)   ###qh:4by4 Hk,so no superconductivity and also assume paramagnetic phase 
...
def ComputeGloc(klist,w,Sg,mu,c,s):
    ...
    Id = identity(4)
    for ikx,iky in klist:  ## qh:here klist is the BZ of the orginal lattice.
        Hk = AssembleHk(ikx,iky,c,s)
        for iw in range(len(w)):
            G_inv = (w[iw]*1j+mu)*Id - Hk - Sg[iw,:,:] ###qh:no translations from cluster self energy to lattice self energy??? 
...
if __name__ == '__main__':
    ...
    mu = params['mu'][0]
    beta = params['beta'][0]
    Nx=64     # mumber of k-points in each direction, in metallic phase you will need to increase this number
    ...
    # create k-mesh
    km = arange(-Nx/2,Nx/2)*2*pi/Nx  ##qh:reduced Brillion zone
    c = cos(km)
    s = sin(km)
    klist = []
    for ikx in range(Nx):
    ...
    if not glob.glob('Sig.out'): # does not exist
        ...
        ss = vstack( (zeros(len(om)), -ones(len(om))*gamma, zeros(len(om)), -ones(len(om))*gamma, zeros(len(om)), -ones(len(om))*gamma) ) ## zero-self energy file.
        data = vstack( (om, ss) )   ##qh:what is vstack??
    else:
    ...
    for it in range(Niter):
        ...
        ind_om = create_log_mesh(om,nom,ntail) ###qh:why??
        w = zeros(len(ind_om))
