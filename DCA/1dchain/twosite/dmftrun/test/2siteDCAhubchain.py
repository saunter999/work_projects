...
def Delta_update():
	...
	for (int i=0;i<masbf.size();i++){
	    for(int j=0;j<Ekls_K0.size();j++){
		gbarK0(i)=gbarK0(i)+1.0/( masbf(i)-Ekls_K0(j)-SigitK0(i) );
		gbarKpi(i)=gbarKpi(i)+1.0/( masbf(i)-Ekls_Kpi(j)-SigitKpi(i));
	    }
	    gbarK0(i)=gbarK0(i)/totk;
	    gbarKpi(i)=gbarKpi(i)/totk;
	    deltaK0(i)=masbf(i)-efK0-1.0/gbarK0(i)-SigitK0(i);
	    deltaKpi(i)=masbf(i)-efKpi-1.0/gbarKpi(i)-SigitKpi(i);
	}
	"""
    weave.inline(code,['EK0imp','EKpiimp','masbf','Ekls_K0','Ekls_Kpi','gbarK0','gbarKpi','totk','deltaK0','deltaKpi','SigitK0','SigitKpi'],type_converters=weave.converters.blitz,force=1,compiler='gcc')
    for ind,iom in enumerate(masbf):
         print>>f,iom.imag,deltaK0[ind].real,deltaK0[ind].imag,deltaKpi[ind].real,deltaKpi[ind].imag
    f.close()
...
if __name__=="__main__":
    ...
    Nocc=1.0;
    t,Uc=1.0,1.0
    Nk=1000 ## 2*Nk is the number of k points in each momentum patch 
    totk=2.0*Nk
    mu=Uc/2.
    mixr=0.9
